...
class memcached_library(object):
	def memcached_get(self,key,ip,port=33210):
		"""查询memcached缓存，Port默认为33210，其他请另行指定
		
		ex:
		| ${keys} | memcached_get | key | 10.16.6.50 | 33210 |
		| ${keys} | memcached_get | key | 10.16.6.50 |
		结果是返回缓存中对应key的值
		"""		
		t=Client(ip,int(port))
		return t.get(key.encode("utf-8"))
		t.close()

	def memcached_del(self,key,ip,port=33210):
		"""删除memcached缓存，Port默认为33210，其他请另行指定
		
		ex:
		| ${keys} | memcached_del | key | 10.16.6.50 | 33210 |
		| ${keys} | memcached_del | key | 10.16.6.50 |
		结果是删除缓存中对应key的值
		"""	
		t=Client(ip,int(port))
		return t.delete(key.encode("utf-8"))
		t.close()

	def memcached_set(self,key,val,exptime,ip,port=33210):
		"""设置memcached缓存，Port默认为33210，其他请另行指定，exptime为缓存有效时间，单位秒
		
		ex:
		| ${keys} | memcached_set | key | val | 10.16.6.50 | 33210 |
		| ${keys} | memcached_set | key | val | 10.16.6.50 |
		结果是设置缓存key的值对应的值为val
		"""	
		t=Client(ip,int(port))
		return t.set(key.encode("utf-8"),val.encode("utf-8"),int(exptime))
		t.close()

#测试代码
if __name__ == '__main__':
	t = Client("10.16.6.50",33210)
	rusult = t.delete("some_key")
	print rusult